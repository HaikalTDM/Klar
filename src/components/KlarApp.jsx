...
import {
    ...
    Share2, Users, UserPlus, Sparkles, Gift, Film, ShoppingCart, Tag, Flag, Mic, Eye, EyeOff, MoreVertical
} from 'lucide-react';
                        ...
                        <>
                            ...
                            >
                                ...
                                <div className={`p-4 border-t space-y-1 ${themeMode === 'custom' ? 'border-[var(--border-color)]' : 'border-slate-200 dark:border-slate-800'}`}>
                                    ...
                                    <button onClick={() => setShowCalendar(true)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all opacity-70 hover:opacity-100"><Calendar size={16} /> Calendar</button>
                    ...
                    <header className={`h-16 flex items-center justify-between px-4 md:px-8 border-b flex-shrink-0 ${themeMode === 'custom' ? 'border-[var(--border-color)]' : 'border-slate-100 dark:border-slate-900'}`}>
                        <div className="flex items-center gap-4">
                            ...
                            <button
                                ...
                                className={`p-2 rounded-lg transition-all opacity-50 hover:opacity-100 ${isZenMode ? 'bg-[var(--accent-color)] text-[var(--bg-color)] opacity-100' : ''}`}
                                        ...
                                        {visibleTasks.map((task) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    >
                                                        <span className={`text-[15px] block transition-all ${isNote ? 'font-normal opacity-90 italic' : 'font-medium'} ${task.isDone ? 'opacity-50 line-through' : ''} ${isFocused ? 'font-semibold' : ''} truncate hover:whitespace-normal hover:overflow-visible`}>{task.text}</span>
                                                                ...
                                                                <div className="relative">
                                                                    {!isNote && !task.isDone && (
                                                                        <button
                                                                            ...
                                                                            className={`p-2 rounded-lg transition-all ${openMenuId === task.id ? 'opacity-100 bg-black/5 dark:bg-white/10' : 'opacity-40 hover:opacity-100'}`}
                                                                        >
                                                                    ...
                                                                    {isNote && (
                                                                        <button onClick={() => startEditing(task)} className="p-2 opacity-50 hover:opacity-100 rounded-lg"><Edit3 size={16} /></button>
                                                                    )}
                                                                        ...
                                                                        {openMenuId === task.id && (
                                                                            <>
                                                                                ...
                                                                                <motion.div
                                                                                    ...
                                                                                    className={`absolute right-0 top-full mt-2 min-w-[160px] z-50 rounded-xl shadow-xl border overflow-hidden p-1 ${themeMode === 'custom' ? 'bg-[var(--card-bg)] border-[var(--border-color)]' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800'}`}
                                                                                >
                                                                                    {!isNote && (
                                                                                        <>
                                                                                            <button
                                                                                                onClick={() => { enterZenMode(task); setOpenMenuId(null); }}
                                                                                                className={`w-full text-left px-3 py-2 rounded-lg text-sm flex items-center gap-2 transition-colors ${themeMode === 'custom' ? 'hover:bg-white/10' : 'hover:bg-slate-100 dark:hover:bg-slate-800'}`}
                                                                                            >
                                                                                                <Maximize2 size={14} /> Zen Mode
                                                                                            </button>
                                                                                            {isFocused && (
                                                                                                <button
                                                                                                    onClick={() => { stopTimer(); setOpenMenuId(null); }}
                                                                                                    className={`w-full text-left px-3 py-2 rounded-lg text-sm flex items-center gap-2 text-red-500 transition-colors ${themeMode === 'custom' ? 'hover:bg-white/10' : 'hover:bg-slate-100 dark:hover:bg-slate-800'}`}
                                                                                                >
                                                                                    ...
                                                                                    <button
                                                                                        onClick={() => { startEditing(task); setOpenMenuId(null); }}
                                                                                        className={`w-full text-left px-3 py-2 rounded-lg text-sm flex items-center gap-2 transition-colors ${themeMode === 'custom' ? 'hover:bg-white/10' : 'hover:bg-slate-100 dark:hover:bg-slate-800'}`}
                                                                                    >
                                                                                        <Edit3 size={14} /> Edit
                                                                                    </button>
                                                                                    ...
                                                                                    {!isNote && (
                                                                                        <button
                                                                                            onClick={() => { setShowTimerSettings(true); setOpenMenuId(null); }}
                                                                                            className={`w-full text-left px-3 py-2 rounded-lg text-sm flex items-center gap-2 transition-colors ${themeMode === 'custom' ? 'hover:bg-white/10' : 'hover:bg-slate-100 dark:hover:bg-slate-800'}`}
                                                                                        >
                                                                                            {Object.values(soundMixer || {}).some(v => v > 0) ? <Volume2 size={14} /> : <VolumeX size={14} />}
                                                                                            Sound Settings
                                                                                        </button>
                                                                                    )}

                                                                                    <div className={`my-1 h-px ${themeMode === 'custom' ? 'bg-white/10' : 'bg-slate-200 dark:bg-slate-700'}`} />
